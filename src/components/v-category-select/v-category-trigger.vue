<template>
  <button
    ...
    type="button"
    class="rg-buttonSty"
    :class="['rg-default-btn', { 'rg-opened': isShow }]"
    :style="{
      minWidth: minWidth,
      minHeight: minHeight,
      maxWidth: buttonTxtMaxWidth,
    }"
  >
    <span class="buttontxt" :style="{ maxWidth: buttonTxtMaxWidth }">{{
      showText || ''
    }}</span>
    <span
      class="rg-iconfont icon-clear rg-clear-btn"
      title="清除选择的项目"
      v-if="value && !disabled"
      @click.stop="clear"
    ></span>
    <span class="rg-caret-down" v-if="!disabled && !value"></span>
  </button>
</template>
